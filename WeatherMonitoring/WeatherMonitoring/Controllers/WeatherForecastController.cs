...
namespace WeatherMonitoring.Controllers
{
    ...
    public class WeatherForecastController : ControllerBase
    {
        ...
        [HttpPut("update")]
        public IActionResult Update([FromQuery] int newtemp, [FromQuery] string newdateTime, [FromQuery] int temp)
        {
            for(int i=0; i < weather.weathers.Count; i++)
            {
                if(weather.weathers[i].TemperatureC == temp)
                {
        ...
        public IActionResult Delete([FromQuery] int tempdel,[FromQuery] string dateTime)
        {
            for(int i = 0; i< weather.weathers.Count; i++)
            {
                ...
                {
                    weather.weathers[i].Date = "";
                    weather.weathers[i].TemperatureC = 0;
                }
            }
            return Ok();
        }
